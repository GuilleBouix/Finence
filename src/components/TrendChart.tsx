...
export const TrendChart = ({ ingresos, gastos }: TrendChartProps) => {
    ...
    const datosGrafico = useMemo(() => {
        ...
        const datosAgrupados = todosLosMovimientos.reduce((acumulador: DatoGrafico[], movimientoActual) => {
            // Formateamos la fecha a día-mes abreviado
            const fechaFormateada = new Date(movimientoActual.fecha).toLocaleDateString('es-ES', { day: '2-digit', month: 'short' });
            
            // Buscamos si ya existe un registro para esta fecha
            const registroExistente = acumulador.find(item => item.name === fechaFormateada);
            
            // Determinamos si el movimiento actual es un ingreso
            const esIngreso = ingresos.some(ingreso => ingreso.id === movimientoActual.id);
        ...
        <div className="bg-[#111] p-4 rounded-xl border border-[#222] flex flex-col h-75">
            ...
            <div className="flex justify-between items-center mb-4 h-8">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-gray-500 text-xs uppercase tracking-widest">Tendencia</h3>
                </div>
            ...
            <div className="h-full w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={datosGrafico}>
                        ...
                        <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#444', fontSize: 9}} minTickGap={20} />
                        ...
                        <Tooltip 
                            contentStyle={{ backgroundColor: '#111', border: '1px solid #222', borderRadius: '8px', fontSize: '10px' }}
